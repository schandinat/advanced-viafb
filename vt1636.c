	...
	u8 index)
{
	...
	viafb_i2c_readbyte(plvds_chip_info->i2c_slave_addr, index, &data);
			      ...
			      *plvds_chip_info, struct IODATA io_data)
{
	...
	viafb_i2c_writebyte(plvds_chip_info->i2c_slave_addr, index, data);
}
...
bool viafb_lvds_identify_vt1636(void)
{
	...
	viaparinfo->shared->chip_info.lvds.i2c_slave_addr = VT1636_LVDS_I2C_ADDR;
	...
	if ((Buffer[0] == 0x45) && (Buffer[1] == 0x33)) {
		viaparinfo->shared->chip_info.lvds.name = VT1636_LVDS;
		return true;
	}
